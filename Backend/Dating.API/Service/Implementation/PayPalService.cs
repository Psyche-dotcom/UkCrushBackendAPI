...
namespace Dating.API.Service.Implementation
{
    public class PayPalService : IPayPalService
    {
        ...
        {
            ClientId = config["PayPalOptions:ClientId"];
            ClientSecret = config["PayPalOptions:ClientSecret"];
            Mode= config["PayPalOptions:Mode"];
        }
        ...
        public async Task<AuthResponse> Authenticate()
        {
            ...
            var httpResponse = await httpClient.PostAsync($"{BaseUrl}/v1/oauth2/token", new FormUrlEncodedContent(content));
