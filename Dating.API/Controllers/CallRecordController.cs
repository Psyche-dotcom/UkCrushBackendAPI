            ...
            try
            {
                ...
                for (int pageNumber = 1; pageNumber <= 500; pageNumber++)
                {
                    for (int pageSize = 1; pageSize <= 500; pageSize++)
                    {
                        ...
                        cacheKey = $"GetTopUserCall_{pageNumber}";
                        _cache.Remove(cacheKey);
                        cacheKey = $"GetAllCamGirlIndividualCallRecordPaginated_{pageSize}_{pageNumber}";
                        _cache.Remove(cacheKey);
                        cacheKey = $"GetUserCall_{addCallRecord.UserId}_{pageSize}_{pageNumber}";
                        _cache.Remove(cacheKey);
                    }
                }
            }
